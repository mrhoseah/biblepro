...
mod ffi {
    ...
    extern "C" {
        ...
        pub fn NDIlib_find_create_v3(p_create_settings: *const NDIFindCreate) -> NDIFindInstance;
        ...
        pub fn NDIlib_recv_create_v3(p_create_settings: *const NDIRecvCreate) -> NDIRecvInstance;
        ...
        if !ptr.is_null() {
            for i in 0..count as usize {
                ...
                if !src.p_ndi_name.is_null() {
                    let name = CStr::from_ptr(src.p_ndi_name)
                        .to_string_lossy()
                        .into_owned();
...
pub fn connect_ndi_source(source_name: &str) -> Result<NdiReceiver, String> {
    ...
    let thread = std::thread::spawn(move || {
        ...
        loop {
            ...
            if ft == ffi::FRAME_TYPE_VIDEO && !vf.p_data.is_null() && vf.xres > 0 && vf.yres > 0 {
                let w = vf.xres as u32;
                let h = vf.yres as u32;
                let bytes = (w * h * 4) as usize;
                let mut data = unsafe { std::slice::from_raw_parts(vf.p_data, bytes).to_vec() };
                ...
                *latest_t.lock().unwrap() = Some(ReceivedFrame {
                    data,
                    width: w,
                    height: h,
                });
...
pub fn connect_ndi_source(_source_name: &str) -> Result<NdiReceiver, String> {
    Err(
        "BiblePro was not compiled with NDI support. Add --features ndi and install the NDI SDK."
            .into(),
    )
}
