...
fn bg_stops(design: &BackgroundDesign) -> Vec<GradientStop> {
    design
        .stops
        .iter()
        .map(|s| GradientStop::new(s.pos.clamp(0.0, 1.0), to_color(&s.color)))
        .collect()
}
...
fn fill_design(pixmap: &mut Pixmap, x: f32, y: f32, w: f32, h: f32, design: &BackgroundDesign) {
    ...
    let shader = match &design.mode {
        ...
        BgMode::Vignette => {
            ...
            let vrev: Vec<GradientStop> = design
                .stops
                .iter()
                .rev()
                .enumerate()
                .map(|(i, s)| {
                    GradientStop::new(
                        i as f32 / (design.stops.len() - 1).max(1) as f32,
                        to_color(&s.color),
                    )
                })
                .collect();
...
fn fill_background(
    ...
    h: f32,
    solid: &Rgba,
    design: &Option<BackgroundDesign>,
) {
    match design {
...
fn wrap<F: Font>(font: &PxScaleFont<F>, text: &str, max_w: f32) -> Vec<String> {
    ...
    for para in text.split('\n') {
        ...
        for word in words {
            ...
            if measure(font, &cand) <= max_w || cur.is_empty() {
                cur = cand;
            } else {
                out.push(cur);
                cur = word.to_string();
...
fn paint_block<F: Font>(
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        paint_line(
            ...
            line,
            x,
            baseline_start + i as f32 * line_h,
            color,
        );
    }
}
...
/// Render a full-frame background using a background design (no text overlay).
pub fn render_background_frame(
    ...
    fill_background(&mut pixmap, w, h, &cfg.background, &Some(design.clone()));
    to_frame(pixmap)
}
...
fn render_card(verse_text: &str, reference: &str, cfg: &PresentConfig) -> Result<Frame, String> {
    ...
    let accent_card = Rgba {
        a: cfg.card_alpha,
        ..cfg.accent_color.clone()
    };
    fill_rounded_rect(
        &mut pixmap,
        card_x,
    ...
    let card_fill = Rgba {
        a: cfg.card_alpha,
        ..cfg.band_color.clone()
    };
    fill_rounded_rect(
        &mut pixmap,
        card_x,
        card_y + cfg.accent_px * 1.5,
        card_w,
        card_h - cfg.accent_px * 1.5,
        cfg.card_radius,
        &card_fill,
    );
...
/// Draw scripture as a lower-third overlay on an existing base frame.
pub fn render_scripture_overlay(
    base: &Frame,
    verse_text: &str,
    reference: &str,
    cfg: &PresentConfig,
    ...
    let v_block_h = v_lines.len() as f32 * v_line_h;
    let gap = cfg.reference_font_size * 0.5;
    let v_baseline = band_top + cfg.accent_px + bold.ascent() + 12.0;
