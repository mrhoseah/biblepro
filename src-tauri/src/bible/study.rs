...
#[tauri::command]
pub fn get_chapter_highlights(db: State<BibleDb>, book_id: i32, chapter: i32) -> Vec<Highlight> {
...
#[tauri::command]
pub fn get_chapter_notes(db: State<BibleDb>, book_id: i32, chapter: i32) -> Vec<Note> {
    let conn = db.0.lock().unwrap();
    let mut stmt = conn
        .prepare(
...
pub fn export_study_set(db: State<BibleDb>, id: i64) -> Result<String, String> {
    let conn = db.0.lock().unwrap();
    let set: Option<(String, String)> = conn
        .query_row(
    ...
    let verses: Vec<serde_json::Value> = stmt
...
pub fn built_in_plans() -> Vec<(&'static str, &'static str, &'static str, i32)> {
    ...
    vec![
        (
            ...
            "Read the entire NT in 3 months, ~3 chapters/day.",
            90,
        ),
        (
            "psalms30",
            "Psalms in 30 Days",
            "All 150 Psalms in one month, 5 per day.",
            30,
        ),
        (
            "proverbs",
            "Proverbs in a Month",
            "One chapter of Proverbs per day for 31 days.",
            31,
        ),
        (
            "genesis",
            "Genesis in 7 Days",
            "The first book of the Bible in a week.",
            7,
        ),
        (
            "sermon",
            "Sermon on the Mount (5 Days)",
            "Matthew 5–7, the heart of Jesus' teaching.",
            5,
        ),
    ]
}
    ...
    passages
        ...
        .map(|(day, label, refs)| {
            ...
            PlanDay {
                ...
                passages: refs
                    .into_iter()
                    .map(|(book_id, book_name, chapter)| PlanPassage {
                        book_id,
...
fn sermon_5_days() -> Vec<(i32, String, Vec<(i32, &'static str, i32)>)> {
    vec![
        (
            1,
            "Beatitudes & Salt and Light".into(),
            vec![(40, "Matthew", 5)],
        ),
...
fn nt_90_days() -> Vec<(i32, String, Vec<(i32, &'static str, i32)>)> {
    ...
    let chapters: Vec<(i32, &'static str, i32)> = all
        .into_iter()
